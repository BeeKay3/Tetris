...
bool init() {
  ...
  if (SDL_Init(SDL_INIT_VIDEO) < 0) {
    ...
    window = SDL_CreateWindow(
        "Tetris", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREENWIDTH,
        SCREENHEIGHT, SDL_WINDOW_SHOWN | SDL_WINDOW_FULLSCREEN);
    if (window == NULL) {
      ...
      if (renderer == NULL) {
        std::cout << "Renderer could not be intialized, error : "
                  << SDL_GetError() << std::endl;
        key = false;
      }
      if (!IMG_Init(IMG_INIT_PNG)) {
        std::cout << "SDL_image could not be intialized, error : "
                  << IMG_GetError() << std::endl;
        key = false;
      }
      if (TTF_Init() < 0) {
        std::cout << "SDL_TTF not initialized, error: " << TTF_GetError()
                  << std::endl;
        key = false;
      }
    }
